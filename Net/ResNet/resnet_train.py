...
def cal_accuracy(predictions, labels):
    predictions = tf.reshape(predictions, [-1, FLAGS.letter_num_per_vc, FLAGS.max_single_vc_length])
    reshaped_labels = tf.reshape(labels, [-1, FLAGS.letter_num_per_vc, FLAGS.max_single_vc_length])
    print predictions
    print labels
    correct_equal = tf.equal(
        tf.argmax(reshaped_labels, axis=2),
        tf.argmax(predictions, axis=2)
    )
    accuracy = tf.reduce_mean(tf.cast(correct_equal, tf.float32))
    return accuracy
